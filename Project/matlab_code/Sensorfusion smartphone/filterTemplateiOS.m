...
%               Louise Olsson
  ...
  % ----------------------- OUR FILTER SETTINGS ------------------------
  t_old = 0;
  Rw = diag([cov(calGyr(:,1)), cov(calGyr(:,2)), cov(calGyr(:,3)) ]);
  Ra = diag([cov(calAcc(:,1)), cov(calAcc(:,2)), cov(calAcc(:,3)) ]);
  Rm = diag([cov(calMag(:,1)), cov(calMag(:,2)), cov(calMag(:,3)) ]);
  g0 = [mean(calAcc(:,1)), mean(calAcc(:,2)), mean(calAcc(:,3))  ]';
  m0 = [0, sqrt(mean(calMag(:,1)).^2 + mean(calMag(:,2)).^2), mean(calMag(:,3))]';
  alpha = 0.01;     % <-- Change alpha here for LP-filter
  Lk = norm(m0);
  ...
  meas = struct('t', zeros(1, 0),...
    ...
    while m.Connected && m.Logging 
      ...
      % Adjust for the error in Azimuth
      % (Matlab may sometimes computes azimuth wrongly)
      %dataOrient(1) = dataOrient(1) + offset;  <-- set offset
      ...
      if m.AccelerationSensorEnabled
          ...
          % Determine if measurement is an outlier
          if norm(acc) > 9.81*1.1 || norm(acc) < 9.81*0.9
      ...
      else % Gyro measurements are NOT available

        % Assume same state/quaternion but increase uncertainty
        [x, P] = tu_qw(x, P, [0,0,0], 1/m.SampleRate, Rw);
        gyr = [NaN, NaN, NaN];
     ...
     if  m.MagneticSensorEnabled
      ...
      % If Mag measurements are not available / sensor is disabled
      else
          mag = [NaN, NaN, NaN]';
      end
      % ------------------------------------------------------------
